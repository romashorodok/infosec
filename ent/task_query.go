...
func (tq *TaskQuery) QueryParticipants() *ParticipantQuery {
	query := (&ParticipantClient{config: tq.config}).Query()
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withParticipants: tq.withParticipants.Clone(),
...
func (tq *TaskQuery) WithParticipants(opts ...func(*ParticipantQuery)) *TaskQuery {
	query := (&ParticipantClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withParticipants = query
	return tq
}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withParticipants; query != nil {
		if err := tq.loadParticipants(ctx, query, nodes,
			func(n *Task) { n.Edges.Participants = []*Participant{} },
			func(n *Task, e *Participant) { n.Edges.Participants = append(n.Edges.Participants, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadParticipants(ctx context.Context, query *ParticipantQuery, nodes []*Task, init func(*Task), assign func(*Task, *Participant)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(task.ParticipantsTable)
		s.Join(joinT).On(s.C(participant.FieldID), joinT.C(task.ParticipantsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(task.ParticipantsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(task.ParticipantsPrimaryKey[0]))
