...
func (bq *BoardQuery) QueryTasks() *TaskQuery {
...
func (bq *BoardQuery) QueryParticipants() *ParticipantQuery {
	query := (&ParticipantClient{config: bq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := bq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (bq *BoardQuery) WithTasks(opts ...func(*TaskQuery)) *BoardQuery {
	query := (&TaskClient{config: bq.config}).Query()
...
func (bq *BoardQuery) WithParticipants(opts ...func(*ParticipantQuery)) *BoardQuery {
	query := (&ParticipantClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (bq *BoardQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Board, error) {
	...
	if query := bq.withTasks; query != nil {
		if err := bq.loadTasks(ctx, query, nodes,
			func(n *Board) { n.Edges.Tasks = []*Task{} },
			func(n *Board, e *Task) { n.Edges.Tasks = append(n.Edges.Tasks, e) }); err != nil {
			return nil, err
		}
	}
...
func (bq *BoardQuery) loadTasks(ctx context.Context, query *TaskQuery, nodes []*Board, init func(*Board), assign func(*Board, *Task)) error {
	...
	query.Where(predicate.Task(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(board.TasksColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.board_tasks
		if fk == nil {
			return fmt.Errorf(`foreign-key "board_tasks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "board_tasks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (bq *BoardQuery) loadParticipants(ctx context.Context, query *ParticipantQuery, nodes []*Board, init func(*Board), assign func(*Board, *Participant)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(board.ParticipantsTable)
		s.Join(joinT).On(s.C(participant.FieldID), joinT.C(board.ParticipantsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(board.ParticipantsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(board.ParticipantsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Participant](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
